...
let package = Package(
    ...
    targets: [
        ...
        // ----------------------------------------------------------------
        // shard-smoke: THROWAWAY harness validating the pipeline model shard
        // (LlamaPipelineShard / LlamaPipelineShardLoader) against REAL Llama
        // weights on ONE machine: loads the full model and a 2-way layer split,
        // runs both forward passes, and asserts the sharded logits match the
        // monolithic logits. No networking, no second machine. NOT a product.
